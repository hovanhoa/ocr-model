...
words = []
with open(os.path.join(os.path.dirname(__file__), 'extraction_words.txt'), encoding="utf8") as f:
    words = f.read().split("\n")

Extraction = Extraction()

for word in words:
    Extraction.add_word(word)

def extraction_text(text):
    return " ".join([word for word in text.split() if Extraction.has_word(word)])
